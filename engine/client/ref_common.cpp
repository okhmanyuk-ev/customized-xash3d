...
{
	static const char *skybox_ext[1] = { "bmp" };
	static const char *skybox_delim[1] = { "" }; // no space for HL style, underscore for Q1 style
	int	i;
	...
	for( i = 0; i <	ARRAYSIZE( skybox_ext ); i++ )
	{
		...
		for( j = 0; j < ARRAYSIZE( skybox_delim ); j++ )
		{
			...
			for( k = 0; k < SKYBOX_MAX_SIDES; k++ )
			{
				...
				if( g_fsapi.FileExists( sidename, false ) || true )
				{
...
void R_SetupSky( const char *name )
{
	...
	for( i = 0; i < SKYBOX_MAX_SIDES; i++ )
	{
		...
		if( !skyboxTextures[i] )
		{
			sky::RequireResource(sidenames[i], [filename = std::string(sidenames[i]), i] {
				auto index = ref.dllFuncs.GL_LoadTexture(filename.c_str(), NULL, 0, TF_CLAMP | TF_SKY);
				extern void R_SetOneSkyboxTexture(int i, int textureIndex);
				R_SetOneSkyboxTexture(i, index);
			});
		}
